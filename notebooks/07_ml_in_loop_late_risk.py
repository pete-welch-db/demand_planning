...
# MAGIC %md
# MAGIC ## 7) Real ML in the loop: Late-delivery risk (changes a decision)
# MAGIC
# MAGIC **Decision this enables**: proactively protect OTIF by identifying **high late-risk orders/lanes** early, then:
# MAGIC - allocate constrained inventory to the right DC/region,
# MAGIC - reserve own-fleet capacity for high-risk lanes,
# MAGIC - reduce reactive premium freight.
# MAGIC
# MAGIC **What this notebook does**
# MAGIC - Builds a labeled training set from `silver_erp_orders` (from the SDP/DLT pipeline)
# MAGIC - Trains a simple model (logistic regression) to predict **late delivery** at order time
# MAGIC - Tracks metrics and registers the model with **MLflow**
...
orders = (
    ...
    .withColumn("is_late", (F.col("actual_delivery_date").isNotNull() & (F.col("actual_delivery_date") > F.col("requested_delivery_date"))).cast("int"))
...
with mlflow.start_run(run_name="late_delivery_risk_logreg") as run:
    ...
    # Also log base rate + simple accuracy at 0.5 threshold
    base_rate = float(train.agg(F.avg("is_late").alias("p")).collect()[0]["p"])
    acc = float(pred.withColumn("correct", (F.col("prediction").cast("int") == F.col("is_late")).cast("int")).agg(F.avg("correct").alias("a")).collect()[0]["a"])
    ...
    # Register into UC model registry
