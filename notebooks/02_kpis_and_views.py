...
# MAGIC
# MAGIC **Run order**
# MAGIC 1) `01_generate_synthetic_data` (writes `bronze_*_raw`)
# MAGIC 2) DLT pipeline `dlt_supply_chain_medallion` (produces `silver_*` and `gold_*`)
# MAGIC 3) `03_forecasting_mlflow` (writes `demand_forecast*`)
...
# COMMAND ----------
# `demand_forecast` is written by `03_forecasting_mlflow` as a Delta table.
